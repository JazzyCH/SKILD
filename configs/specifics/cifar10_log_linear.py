"""
CIFAR-10 / NCSN++ / log-linear schedule.

Settings used to produce the log-linear-schedule entry in Table 1 of the
paper. Identical architecture to ``cifar10_linear.py``; the only change
is the lambda schedule (linear in log-space rather than wave-number).
"""
...
def get_config():
    ...
    # ---- diffusion schedule (log-linear; paper Appendix B.2) ----
    model.num_scales = 1000
    model.lambda_i = -3.75            # log10(lambda_i) at step 1
    model.lambda_f = -2.0             # log10(lambda_f) at step N
    model.snr_threshold = 1e-4
    model.kc = 31.2
    model.lambda_schedule = "log_linear"
